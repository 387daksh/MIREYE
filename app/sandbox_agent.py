...
DILIGENCE_TOOL_DEFINITIONS = [
    ...
    _tool("get_user_evidence", "Read durable user-submitted project evidence. Every record remains pending human source and scope review.", {"project_id": {"type": "string"}}, ["project_id"]),
    ...
    for subject, outcome in claims:
        ...
        if not matching:
            domain_matching = [
                item for item in coverage
                if normalized in {
                    re.sub(r"[^a-z0-9]+", " ", str(item.get("domain", "")).casefold()).strip(),
                    re.sub(r"[^a-z0-9]+", " ", f"{item.get('domain', '')} readiness".casefold()).strip(),
                }
            ]
            if domain_matching and any(item.get("status") != outcome.upper() for item in domain_matching):
                summary = "; ".join(f"{item['title']} is {item['status']}" for item in domain_matching)
                return f"{domain_matching[0]['domain']} has mixed requirement outcomes: {summary}."
            matching = domain_matching
        if not matching or any(item.get("status") != outcome.upper() for item in matching):
...
class SandboxAgent:
    ...
    async def _execute_project_tool(
        ...
        *,
        ...
        if name == "get_user_evidence":
            SandboxToolExecutor._only(arguments, {"project_id"}, {"project_id"})
            return self.diligence.user_evidence(project_id)
